...
def number_films_in_category(category_id:int)->pd.DataFrame:
    ...
    '''
    if isinstance(category_id, int):
        d_number_category = """select category.name as "category", count(film.title)
                    from film 
                    ...
                    """
        df = pd.read_sql(d_number_category, con=connection)
...
def number_film_by_length(min_length: Union[int,float] = 0, max_length: Union[int,float] = 1e6 ) :
    ...
    if isinstance(min_length,(int, float)) and isinstance(max_length,(int, float)) and min_length < max_length:
        d_number_length = """select film.length ,count(film.length) 
                 from film 
                 where film.length 
                 between """+str(min_length) + " and "+ str(max_length) +\
                "group by film.length"
        df = pd.read_sql(d_number_length, con=connection)
        return df
    else:
        return None
...
def client_from_city(city:str)->pd.DataFrame:
    ...
    if isinstance(city, (str)):
        d_client = """select city.city, customer.first_name, customer.last_name
                        from customer 
                        inner join address on customer.address_id = address.address_id 
                        inner join city on address.city_id = city.city_id 
                        where city.city in ('""" + str(city) + """')
                        order by city asc"""
        df = pd.read_sql(d_client, con=connection)
        return df
    else:
        return None
...
def avg_amount_by_length(length:Union[int,float])->pd.DataFrame:
    ...
    if isinstance(length, (int, float)):
        ...
        d_avg = """select film.length as "length", avg(payment.amount) 
                    from payment 
                    ...
                    """
        df = pd.read_sql(d_avg, con=connection)
        return df
    else:
        return None
...
def client_by_sum_length(sum_min:Union[int,float])->pd.DataFrame:
    ...
    if isinstance(sum_min, (int, float)):
        d_client_length = """select customer.first_name, customer.last_name, sum(film.length)
                        FROM rental 
                        inner join inventory on rental.inventory_id = inventory.inventory_id 
                        inner join film on inventory.film_id = film.film_id
                        ...
                        """
        df = pd.read_sql(d_client_length, con=connection)
        return df
    else:
        return None  
...
def category_statistic_length(name:str)->pd.DataFrame:
    ...
    if isinstance(name, (str)):
        d_category_length = """select category.name, avg(film.length), sum(film.length), min(film.length), max(film.length) 
                    from film 
                    inner join film_category on film.film_id = film_category.film_id 
                    inner join category on film_category.category_id = category.category_id
                    ...
                    order by category.name asc
                    """
        df = pd.read_sql(d_category_length, con=connection)     
